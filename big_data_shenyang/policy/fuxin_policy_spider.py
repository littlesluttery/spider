...
def total_page_numbers():
    url = "http://kjj.fuxin.gov.cn/newslist.jsp?id=12352"
    r = requests.get(url,headers=headers)
    try:
        if r.status_code == 200:
            r.encoding='utf-8'
            html = etree.HTML(r.text)
            total_page_number = html.xpath("//div[@class='page-inner']/text()")
...
def page_urls(url):
    ...
    try:
        if r.status_code == 200:
            r.encoding = 'utf-8'
            html = etree.HTML(r.text)
            urls = html.xpath("//div[@class='list1-line']/a/@href")
            return urls
        else:
            return None
    except ConnectionError:
...
def page_details(url):
    ...
    try:
        if r.status_code == 200:
            ...
            html = etree.HTML(r.text)
            # 标题
            title = html.xpath("//div[@class='DetailcontentTile']/text()")
            title = ''.join(title).strip()


            # 发布日期：release_data
            release_data = html.xpath("//p[@class='DetailcontentTime']/span[1]/text()")
            release_data = ''.join(release_data).strip()
            release_data = re.findall("(\d+-\d+-\d+)",release_data)
            release_data = ''.join(release_data).strip()
            # 来源
            source = html.xpath("//p[@class='DetailcontentTime']/span[2]/text()")
            ...
            """
            details = html.xpath("//div[@id='Detailcontent']//p//text()")
            details = ''.join(details).strip()
...
def main():
    ...
    for i in range(1,int(total_page_number)+1):
        url = 'http://kjj.fuxin.gov.cn/newslist.jsp?id=12352&&pn={}'.format(i)
        urls = page_urls(url)
        for url in urls:
            page_details(url)
